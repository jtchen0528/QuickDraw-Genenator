...
if __name__ == '__main__':
    ...
    parser.add_argument("--target_root", '-t', type=str, default='Trained_models',
                        help="root to store trained model")
    ...
    class QD_Dataset(data.Dataset):
        def __init__(self, mtype, root='Dataset'):
            data, target, self.num_classes = load_dataset(root, mtype)
            ...
            self.label_frame = target
            print(self.data_frame.shape)
            print(self.label_frame.shape)
            print("Dataset "+mtype+" loading done.")
            print("*"*50+"\n")

        def __getitem__(self, index):    
            imgs = self.data_frame[index]
            labels = self.label_frame[index]
            return imgs, labels
    ...
    generate_dataset(vfold_ratio=0, max_samples_per_class=sample_size,
                     show_imgs=False, frame_size=28)
    ...
    for epoch in range(train_epoch):
        ...
        for i, (x_, y_) in enumerate(train_loader):
            ...
            print('epoch [%d/%d] batch [%d/%d] - loss_d: %.3f, loss_g: %.3f' % ((epoch + 1), train_epoch, i, len(train_loader), torch.mean(torch.FloatTensor(D_losses)),

        ...
        for epoch in range(epochs):
            for i, imgs in enumerate(dataloader):
                ...
                if batches_done % interval == 0:
                    ...
                    if save_model:
                        torch.save(generator.state_dict(), './models/' + name + '/model_bee_' + str(batches_done) + '.pytorch')

        if logging:
            with open("./logs/g_loss_" + name + ".txt", "w") as output:
                output.write(str(G_LOSS))
            with open("./logs/d_loss_" + name + ".txt", "w") as output:
